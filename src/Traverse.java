...
public class Traverse {
  ...
  private static boolean reachable(Map<Integer,Set<Integer>> graph, int start, int end, Set<Integer> seen){
    ...
    if(seen.contains(start) || graph == null || !graph.containsKey(start) || graph.get(start) == null) return false;
    seen.add(start);

    for (int neighbor : graph.get(start)) {
      if(reachable(graph, neighbor, end, seen)) return true;
    }
  ...
  private static void printGossipers(Person person, Set<Person> seen){
    if(person == null || seen.contains(person)) return;
    seen.add(person);
